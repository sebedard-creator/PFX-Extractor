...
4. **Assemblage Pro Tools :** À partir des WAV présents dans `work/processed`, l'interface peut produire une livraison autonome contenant la session `.ptx` et son dossier `Audio Files`. Chaque placement vient du timestamp BWF du média; il ne s'agit donc pas d'une décision globale basée sur la durée du clip. Le tout est livré dans une archive ZIP.
5. **Flush (Nettoyage) :** L'utilisateur peut vider le cache local, les livraisons générées et les répertoires sur Google Drive en un clic depuis l'interface Gradio pour préparer une nouvelle session de travail.
