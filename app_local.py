...
def download_processed_zip(template_file, session_name):
    try:
        ...
        if drive_result["count"] == 0:
            raise gr.Error(
                ...
                f"{drive_auth.PARENT_FOLDER_NAME}/"
        ...
        return (
            ...
            f"Erreur: {exc}\n\n"
            "Les WAV déjà téléchargés restent dans le dossier de travail. "
            "Aucune livraison Pro Tools incomplète n'a été publiée."
        ), None
    ...
    status = (
        ...
        f"{_plural(drive_result['count'], 'fichier')} récupéré(s) depuis Drive.\n"
...
with gr.Blocks(title="PFX Extractor - Drive Bridge", css=CSS) as demo:
    ...
    with gr.Row():
        download_btn = gr.Button("Télécharger les fichiers traités en ZIP", variant="primary", size="lg")
    ...
    with gr.Accordion("⚙️ Paramètres avancés", open=False):
        gr.Markdown(
            "Le bouton de téléchargement crée automatiquement la session Pro Tools avec "
            "le `template.ptx` local placé à la racine de l'application. Ce fichier "
            "propriétaire n'est pas versionné. Vous pouvez fournir une autre template "
            "et un nom de session pour un export ponctuel."
        )
        with gr.Row():
